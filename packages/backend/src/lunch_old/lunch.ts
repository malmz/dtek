import { getLunchDate, getStartOfWeek } from '../date.js';
import * as karen from './karen.js';
import * as linsen from './linsen.js';
import { startOfDay, isEqual, isWeekend, isAfter } from 'date-fns';
...
class LunchProvider {
  ...
  public getMenu(name: string, date: Date): Menu {
    ...
    const todaysMenu = menu.days.find((day) =>
      isEqual(startOfDay(day.date), startDate)
    );
    if (!todaysMenu) {
  ...
  async cacheAll() {
    ...
    for (const [name, resturant] of this.#resturants) {
      ...
      const fetcher = isWeekend(new Date())
